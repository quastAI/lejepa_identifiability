...
| **Camera look-at authoring needs an up-vector fallback for near-vertical views** | `idtb/scenegen/stage_v1.py::_look_at_transform`. `Gf.Matrix4d.SetLookAt` with world-up `(0,0,1)` is degenerate when the view direction is nearly parallel to it — confirmed empirically (not just in theory): the original rig's near-top-down camera authored a literal-`FLT_MAX` transform before this was caught by `usd-core` running locally and fixed with a `(0,1,0)` fallback. Kept even though the current head-stereo rig's ~50° angle is nowhere near this case, so any future camera added here is still covered. A reminder that "no local Isaac runtime" (§1) doesn't mean no local bugs to catch in the parts that don't need one. |
| **`stage_v2_room.usda` composes stage v1 by *reference*, not by extending `stage_v1.py`'s build function or sublayering it in** | docs/PLAN.md Phase 2c. A reference is the idiomatic arc for pulling in an encapsulated, already-complete asset — `materials.py`'s docstring already establishes this reasoning for the Franka. `stage_v1_tabletop.usda` stays exactly what Phase 2 shipped and tested, unmodified, whether or not the room ever wraps it, and Phase 3 rework's runtime absolute paths (`/World/Table`, `/World/Cameras/CameraL`, …) survive unchanged into stage v2 because the reference's target primPath (`/World`) equals stage v2's own default prim path. Confirmed locally: a *reference* (unlike an anonymous sublayer) needs a real anchoring file on disk to resolve a relative asset path against — `write_stage_v2_room_usda` builds with an absolute reference, then repoints it to a relative one only at export, the same repoint-at-export step `write_stage_v1_usda` already does for its materials sublayer (and produces the exact same class of benign "could not open asset" warning on the in-memory stage while doing it). |
| **Room shell: 3×3 m floor, 2.5 m ceiling, centered on `CUBE_XY`, not the world origin** | `idtb/scenegen/stage_v2_room.py`. Sized to clear every camera eye in the rig with a real margin (≥0.5 m to every wall/floor/ceiling — `tests/test_scenegen.py::test_room_encloses_every_camera_eye_with_margin`), not just to exist; centered on the table's own centering point since the table, not the world origin, is what the room needs to actually contain. Ordinary small-room scale, not oversized: briefly grown to 8×8×4 while the (now-retired) 3-camera rig sat 2–2.65 m from the table, then sized back down once the head-stereo rig replaced it at ~0.8 m out. Floor/ceiling/walls are matte `UsdPreviewSurface` boxes (`materials.py::ROOM_PBR`), not load-bearing for any latent — purely a backdrop. |
| **Camera rig: a single head-mounted stereo pair, not 3 independent monocular views** | docs/PLAN.md Phase 2c, `idtb/scenegen/stage_v1.py`. Replaces the original `Camera1`/`Camera2`/`Camera3` rig. Matches how real humanoids actually see (Figure 01/02, 1X NEO, Unitree H1/G1 all use head-mounted stereo as their primary/sole vision sensor) rather than an arbitrary multi-viewpoint rig or a wrist-mounted eye-in-hand camera — the latter would move with the arm's own `full`-latent joints, confounding monocular scale cues with `cube.size` exactly as this rig exists to avoid. `CameraL`/`CameraR` sit at one humanoid-plausible lean-over-the-table position (~50° below horizontal, ~0.8 m from the cube), separated by a 70 mm RealSense-D435-class baseline, giving real depth-from-disparity to disambiguate `cube.size` from distance — something resampled camera pose (`cam.jitter`) could never do. Trades away the old rig's multi-view occlusion mitigation entirely (§5.3, §11) — an accepted, explicitly deferred risk, not silently dropped: occlusion can't be measured at all until Phase 3 rework spawns the Franka into this still-robot-less static scene, regardless of camera count. |
| **Every authored camera gets an explicit `clippingRange` and a realistic FOV, not the `UsdGeom.Camera` schema fallbacks** | `idtb/scenegen/stage_v1.py::_define_camera`. The fallback `clippingRange` is `(1, 1000000)` — a 1 m near plane, confirmed on the pod clipping the near foreground of a rig whose cameras sit under 1.2 m from their target; now explicit at `(0.01, 100.0)` m. The fallback lens (50 mm focal length / 20.955 mm aperture) is a ~24° horizontal FOV — confirmed by ray-casting the frustum against the table plane to see only a ~0.5×0.5 m patch from the head-stereo rig's position, tighter than the table itself; `_define_camera` now solves focal length for a real RealSense D435 RGB-class ~69° horizontal FOV instead (~2×2 m footprint at the same position), keeping the aperture as authored rather than hardcoding a second constant. |
| **The dome light is not re-authored for the room; it stays textureless, deferred, per §3.2** | docs/PLAN.md Phase 2c. `stage_v1.py`'s composed-in `DomeLight` already anticipated a room-consistent HDRI might eventually matter; no resolved Nucleus HDRI asset path exists yet, so the same "no premature pins" reasoning that kept it textureless in Phase 2 keeps it textureless here too — not silently assumed fixed by the room's existence. Whether a real HDRI, or the current neutral-color fallback, is actually correct once walls exist to bounce off is exactly what the Phase 2c pod session (visual check) and a re-run of Phase 2b's `table.albedo`/`table.roughness`/`light.azimuth_elevation` knob checks against the room are for. |
...
| ~~Which `full`/`style` knobs are writable at all, and through which API call~~ | **Resolved against scene v1 (§7.6) and scene v2 (§7.7).** `cube.size`, `cube.hue`, `light.intensity`, `light.warmth`, `table.roughness`, `table.albedo`, and (as of scene v2) `light.azimuth_elevation` are all writable and bitwise-deterministic. `cam.jitter` writes and is strongly responsive but has a residual non-determinism that scales with scene complexity — small against scene v1, confirmed room-caused and much larger against scene v2 — tracked in §11. | — |
| **Squash radii for every `full` and `style` handle** | Measured ranges from the same spike, plus scene v1/v2 for anything table-relative. `cube.size`'s upper bound is already pinned by the *measured* gripper aperture (§5.2.2); the determinism blocker (§7.5/§7.6/§7.7) is resolved for every handle now, including `table.roughness` and `light.azimuth_elevation` — only `cube.x`/`cube.y`'s radius (needs the Franka actually spawned) and `cam.jitter`'s residual remain open. | Phase 3 rework |
...
- **`IsaacSceneBackend`** (`src/idtb/sim/scene.py`, plus `app.py`/`render.py`/`writer.py`) — real, lazy-imports Isaac, runs remotely. **Built and run on the pod (§3.1); tier-1 contract suite green.** `bind()` declares support for exactly the roles §7.2/§7.5 confirmed against the *old spike scene* and refuses `light.azimuth`/`light.elevation`/`table.roughness` via `UnsupportedRoleError`. §7.4's closing note already predicted this backend gets rewritten once scene v1 replaces the spike scene's light rig and table material — **§7.6/§7.7 are that re-test**: `table.roughness` and `light.azimuth_elevation` are both resolved clean against scene v1/v2, so this refusal list is stale, updated in Phase 3 rework alongside the rest of the rewrite.
...
Everything in `base` moves through `write_joint_state_to_sim` /
`write_root_state_to_sim`, the two paths Spikes 1–2 verified (§7.2). Cube size
is a geometry write and cube colour is a material write — a different
mechanism, now resolved: both are writable, bitwise-deterministic, and
responsive against scene v1 and scene v2 (§7.6/§7.7), via
`spikes/spike_dynamic_attrs.py` (§7.5).
...
the pair. Every write path below is now verified, against both scene v1 (§7.6) and
scene v2 (§7.7) — the two re-gates the §7.5 attribute spike drove.
...
| `light.azimuth`, `light.elevation` | Key-light direction — "lighting jitter" | light prim transform | Shadow direction is the most visually salient style cue, and therefore the strongest test of invariance. **Resolved, §7.7**: writable, bitwise-deterministic, and — against scene v2's room, which gives the shadow it casts real surfaces to land on — no longer the practically-dead knob §7.6 found against room-less scene v1 (`mad_vs_base` went from `0.00012` to `1.69`, in line with every other `style` knob). Fixed by the room, not by changing the light itself. |
| `cam.jitter.*` | Per-capture camera pose jitter about the nominal view | `set_world_poses_from_view`, **per capture** | Spike 1 aimed the camera once, at boot. Per-sample re-aiming is a different usage pattern and gets its own check. Radius stays small enough that §5.3's occlusion geometry is not substantially changed. **Open finding, §7.7/§11**: strongly responsive and bitwise-clean against scene v1, but picks up a residual non-determinism against scene v2's room (confirmed room-caused via a controlled A/B, not yet root-caused) — not blocking since this is a `style`-only latent, but worth resolving before trusting it at dataset-generation scale. |
| `table.albedo` | Table material colour | USD material attribute (`PreviewSurface.diffuseColor`, grey) | Continuous, squashes cleanly. **Resolved clean, §7.6/§7.7**: bitwise-deterministic and strongly responsive against both scene v1 (`mad_vs_base` 14.82) and scene v2 (11.84) — the §7.5-era caveat about a weak effect was against the old spike scene's flat material, not this one. |
| `table.roughness` | Table material roughness | USD material attribute (`PreviewSurface.roughness`) | Continuous. **Resolved clean, §7.6/§7.7**: writable, bitwise-deterministic, and clearly responsive against both scene v1 (`mad_vs_base` 1.86) and scene v2 (0.32) — the old scene's exactly-zero effect was specific to its flat material/lighting, not a structural limit on the knob itself. |
...
> **Superseded, 2026-09-21 (docs/PLAN.md Phase 2c):** the paragraph above described scene v1's original 3-camera rig, which implemented (1)+(2) directly. That rig is retired — see below for what replaced it and why occlusion is now an open risk rather than a mitigated one.

**Scene v1's camera rig is now a single head-mounted stereo pair** (`CameraL`/`CameraR`, a 70 mm RealSense-D435-class baseline, `idtb/scenegen/stage_v1.py`), not the multi-view rig the paragraphs above describe. This was a deliberate redesign toward realistic humanoid sensing — real humanoids (Figure 01/02, 1X NEO, Unitree H1/G1) use head-mounted stereo as their primary or only vision sensor, not several independent third-person viewpoints, and not a wrist-mounted eye-in-hand camera (which would move with the arm's own `full`-latent joints, confounding monocular scale cues with `cube.size` exactly the way this rig exists to avoid). The stereo baseline gives real depth-from-disparity, which resampled camera pose (`cam.jitter`) never could.

This trades away mitigation (1) [multi-view] entirely, and only partially buys (2): the head is angled the way a humanoid would actually lean over the table (~50° below horizontal, chosen for realism), not tuned for occlusion the way the retired near-top-down `Camera3` was. Mitigation (4) [measure it] is still unbuilt. **Occlusion is therefore an accepted, explicitly deferred risk against this rig, not a solved problem** — tracked in §11. It cannot be measured yet regardless: the Franka is not spawned in this static scene at all (Phase 3 rework's job), so there is nothing to occlude the cube with until then.

`test_camera_rig_is_a_realistic_head_mounted_stereo_pair` (`tests/test_scenegen.py`) is the current structural check on the rig (baseline distance, shared target, level eyes) — the old multi-axis-parallax test it replaced no longer applies to a rig that's deliberately one viewpoint. Wiring `CameraL`/`CameraR` into `IsaacSceneBackend` as two plain `Camera` sensors (not `TiledCamera` — see docs/PLAN.md Phase 3 rework for why) is still open.

> **Multi-view earned its place empirically, not just theoretically — and that finding still stands even though the fix it prompted was retired.** Phase 3's single-camera contract suite measured `cube.y`'s render signal at roughly a third of `cube.x`'s (mean abs diff `0.039` vs `0.126` for the same-magnitude move) — a real but weak signal, an artefact of that one camera's oblique angle putting Y-motion closer to its own viewing axis. Worse: under enough simultaneous attribute writes in one long session, that already-weak signal was observed dropping to exactly `0.0`. Adding a second, differently-angled camera (the old `Camera2`) was one way to fix the *magnitude* problem; the new head-stereo rig is a different one (angled closer to top-down than the original `Camera1`, which may or may not symmetrize x/y sensitivity the same way — untested). **Whether `cube.y`'s signal is still weak from this new single viewpoint is an open, deferred question (§11), not assumed fixed or assumed broken.** Neither this nor the exact-zero drop is explained by the rig change alone.
...
>
> **`cube.y` specifically stayed a real, if weak, residual finding after that fix** — not zero, but roughly a third of `cube.x`'s magnitude and, in one long session, still observed hitting exactly `0.0` under enough simultaneous attribute writes. §5.3 and §11 track this as a scoped, open item, not a closed one. **Revised, 2026-09-21**: the multi-view rig this used to point to as the expected fix is retired (§5.3) — whether the new single-viewpoint head-stereo rig's different angle helps, hurts, or does nothing to this signal is untested, not assumed either way.
...
| **`standard`** | Main dataset generation: provably deterministic at tractable cost. | **`/rtx/rendermode=PathTracing`, `/rtx/pathtracing/spp=1`, `/rtx/pathtracing/totalSpp=64`, `/rtx/pathtracing/optixDenoiser/enabled=0`, `/rtx/resetPtAccumOnAnimTimeChange=True`** | **Measured, §7.2 Spike 1+3, §7.5 Spike 5 rounds 2–3.** `order_independent_bitwise`/`back_to_back_bitwise` both `true` for `base` (Spike 1) and for `cube.size`/`cube.hue`/`light.intensity`/`light.warmth`/`cam.jitter`/`table.albedo` (Spike 5); converges inside a single external render call. The fifth key is what fixed everything except `light.azimuth_elevation` and `table.roughness` — see §7.5. |
...
- **Camera:** fixed intrinsics, a single head-mounted stereo pair per §5.3 (`CameraL`/`CameraR`, 70 mm baseline, ~69° horizontal FOV — a real RealSense D435 RGB-class lens, not the schema's ~24° telephoto fallback, confirmed on the pod after the fallback FOV cropped the render to a ~0.5×0.5 m patch of the table). Intrinsics logged to dataset metadata and **held constant**; extrinsics carry a small `style` jitter about the nominal pose, bounded so §5.3's (now-open, not mitigated) occlusion geometry is not substantially changed further.
...
> **Closing note, cited throughout §3/§4/§9/docs/PLAN.md as "§7.4 already predicted":** `IsaacSceneBackend`/`writer.py`/`render.py` (§4.3, §4.4) were built and verified (Phase 3, §9) against Spikes 1–5's own inline scene — one `DistantLight`, a flat material-test table, a single camera. Once scene v1's real PBR materials, HDRI dome, area lights, and camera rig replace that scene (Phase 2), the backend needs re-verifying against the real thing, not assumed to transfer unchanged. §7.6/§7.7 are that re-test, run against scene v1 and scene v2 respectively; Phase 3 rework is where `ROLE_WRITE_PATHS` and the rest of the backend actually get updated to match.
...
including or excluding it as a `style` latent (`docs/PLAN.md` Phase 2b has the checklist item).
...
ground plane so it renders in-frame, and `run_knob_check` covers both the
same way as everything else.

The first pod run used the Round-1 configuration by mistake
(`--reset-pt-accum-on-time-change` wasn't passed) and reproduced the
already-diagnosed back-to-back noise on `cube.hue`/`light.intensity`/
`light.warmth`/`cam.jitter` right alongside the two new checks — confirmed
from that run's own `render_product_attribute_audit`, which showed
`resetPtAccumOnAnimTimeChange: false`. The second run, with the flag, is the
real verdict: those four are back to bitwise-clean, confirming the table
addition disturbed nothing already resolved, and:

- **`table.albedo`: clean.** Bitwise-deterministic (`order_independent_mad`
  and `back_to_back_mad` both exactly `0.0`) and responsive
  (`mad_vs_base = 0.00014`, nonzero) — real, but roughly 100–1000× smaller
  than every other knob's effect size (`cube.hue`'s is `0.16`,
  `light.intensity`'s is `27`), consistent with a small, off-centre table
  patch in a 128×128 frame rather than a broken write. Joins the resolved set.
- **`table.roughness`: blocked, a second dead knob.** Writes and reads back
  exactly (error `1.2×10⁻⁸` against a `1×10⁻³` atol) and is fully
  bitwise-deterministic — but `states_distinguishable_mad` is exactly `0.0`:
  varying it `0.5 → 0.95` changes no pixel at all. Not noise this time
  (everything here is bitwise-clean); the leading theory is that no visible
  specular highlight lands on the table from this camera angle in this
  scene — roughness only sharpens or blurs a specular response that isn't
  there to begin with. Recorded next to `light.azimuth_elevation`: **blocked,
  carried forward, not dropped.** Both dead knobs may be specific to this
  spike scene's flat `DistantLight` + plain `PreviewSurfaceCfg` material;
  scene v1's real PBR materials and HDRI/area lighting (§7.4) are a
...
### 7.7 Spike 5 re-gate against scene v2 (room) — resolved, 2026-09-22 (docs/PLAN.md Phase 2c)

`spike_dynamic_attrs.py` was retargeted again, from `stage_v1_tabletop.usda`
directly to `stage_v2_room.usda` (the room-inclusive stage), and re-run under
the same `standard` recipe — this time also against Phase 2c's redesigned
camera rig (§5.3, §3.1: a single head-mounted stereo pair, not the retired
3-camera rig §7.6 measured against). Same headline shape as §7.6: 18 passed,
2 failed. `motion_blur_under_teleport_no_step` is the same by-design non-issue
as every prior round. The other FAIL is `cam.jitter` again — but at a
dramatically different magnitude, investigated below rather than assumed to
be the same finding just restated.

**Every attribute-write knob stayed (or became) bitwise-clean, and one — `light.azimuth_elevation` — went from practically dead to genuinely usable:**

| Knob | `mad_vs_base` (scene v1, §7.6) | `mad_vs_base` (scene v2, this re-gate) | Bitwise deterministic | Verdict |
|---|---|---|---|---|
| `cube.size` | 0.61 | 0.96 | yes | clean |
| `cube.hue` | 0.46 | 0.48 | yes | clean, unchanged |
| `light.intensity` | 1.15 | **15.07** | yes | clean, ~13× stronger — more surrounding surfaces for it to visibly light |
| `light.warmth` | 0.33 | **4.63** | yes | clean, ~14× stronger, same reason |
| `table.albedo` | 14.82 | 11.84 | yes | clean, same order of magnitude |
| `table.roughness` | 1.86 | 0.32 | yes | clean, still solidly live — magnitude shifted, not the knob's aliveness |
| `light.azimuth_elevation` | 0.00012 | **1.69** | yes | **resolved — no longer a practically-dead knob.** ~14,000× stronger than scene v1's measurement, now squarely in the same range as every other live `style` knob. The room gives the `KeyLight`'s rotation real surfaces to change the look of (shadow direction/shape against walls and floor), which the room-less scene v1 never had — confirms the exact mechanism §7.6 predicted might fix this ("worth revisiting with a larger or more central key light") without actually needing to touch the light itself |
| `cam.jitter` | 11.36 | 3.43 | **no — large residual** (`back_to_back_mad` **0.4427**, vs. `4.07×10⁻⁵` in §7.6) | real, strong sensitivity, same as before; determinism regressed by ~4 orders of magnitude — investigated below, not assumed to be "the same small finding" just because it has the same name |

**The `cam.jitter` regression was isolated with a controlled A/B, not assumed.** Before writing this up as "the room broke camera determinism," a `--stage v1|v2` toggle was added to `spike_dynamic_attrs.py` specifically to test one thing: does this change because of the room, or because of Phase 2c's *other* change in the same phase (the camera-rig redesign, §5.3)? Re-running against `stage_v1_tabletop.usda` with the **exact same camera position** (still `CameraL`'s eye/target, still the new stereo rig's placement) dropped `back_to_back_mad` straight back to `2.035×10⁻⁵` — the same tiny residual §7.6 already found and tracked, not zero, but four orders of magnitude smaller than with the room. **This isolates the room itself as the cause, independent of the camera-rig change that happened to land in the same phase.** Order-independence stayed bitwise-clean (`0.0`) in both cases — it is specifically *repeating* the identical jittered pose that shows the room-dependent noise, not camera writes in general. Not yet root-caused beyond that: a plausible mechanism is that the room's added geometry increases path-traced indirect-light/edge-sampling variance specifically at this jittered view, and whatever resets accumulation between writes doesn't fully compensate for it — but that is a hypothesis, not a confirmed mechanism. Tracked in §11 as a materially larger version of the same open risk, not a new category of bug.
...
| **2** | Scene v1 | `stage_v1_tabletop.usd`: table, cube, PBR materials, HDRI dome + 2 area lights, camera rig. **Done, 2026-09-21** — authored by `src/idtb/scenegen/` and confirmed rendering on the pod under `debug`: `Camera1`/`Camera2` look correct (table + cube visible, reasonably lit); `Camera3` (near-top-down) renders near-black, not yet root-caused. The Franka is deliberately *not* in this static file — spawned dynamically in Phase 3 rework instead. **Camera rig later fully replaced in Phase 2c** — `Camera1`/`Camera2`/`Camera3` retired in favor of a single head-mounted stereo pair (`CameraL`/`CameraR`), so `Camera3`'s near-black finding no longer applies to any camera that exists; see §5.3, §9 Phase 2c, §11. | 1–2 days |
| **2b** | Spike 5 — attribute writes | `spikes/spike_dynamic_attrs.py` (§7.5/§7.6): does each `full`/`style` knob write, read back, move pixels, and stay bitwise deterministic under `standard`? **Done, 2026-09-21**, re-gated against scene v1 (not just the spike scene): `table.roughness` is no longer a dead knob, two real bugs found and fixed (`cube.size` units, materials outside the default prim), `cam.jitter`'s small residual non-determinism tracked but not blocking (§7.6, §11). | 1 day |
| **2c** | Scene v2 (room) — **reordered ahead of Phase 6, 2026-09-21** | USD reference of an empty room shell around stage v1. **Done, 2026-09-22** — `idtb/scenegen/stage_v2_room.py` composes `scenes/stage_v1_tabletop.usda`'s `/World` onto its own `/World` by reference (final size 3×3 m floor, 2.5 m ceiling, centered on `CUBE_XY`, clears every camera eye with ≥0.5 m margin — §3.1), regenerated into `scenes/stage_v2_room.usda`, structurally verified via `usd-core` (`tests/test_scenegen.py`). Dome light stays textureless, deferred (§3.1/§3.2) — not re-authored. **Camera rig fully redesigned during this phase's pod session**, not just the room: the original `Camera1`/`Camera2`/`Camera3` rig is retired in favor of a single realistic head-mounted stereo pair (`CameraL`/`CameraR`, §3.1, §5.3) — two real bugs found and fixed along the way (schema-default `clippingRange` clipping the near foreground; a ~24° telephoto default FOV cropping the render to a ~0.5×0.5 m patch, fixed to a realistic ~69° RealSense-D435-class FOV), plus the camera-rig redesign itself (a design decision, not a bug). Pod-confirmed clean (`docs/pod_sessions/2026-09-21_scene_v2_view/`). **Phase 2b's knob re-gate re-run against the room (§7.7)**: `table.albedo`/`table.roughness` stayed clean, `light.azimuth_elevation` went from practically dead to genuinely usable (~14,000× stronger), and `cam.jitter`'s already-tracked residual non-determinism escalated substantially — isolated via a controlled A/B to the room specifically, not the camera-rig change, and tracked in §11 as open, not blocking. **Still open, deferred to Phase 3 rework**: `cube.y`'s signal strength against the new rig's angle, and occlusion (untestable until the Franka is spawned). | 4–6 days |
| **3** | Backend seam + real backend | `SceneBackend` protocol and `MockSceneBackend`, designed against the spike's measurements rather than against documentation; then `IsaacSceneBackend`: handle resolution by name, state writer, read-back assertions confirming every write landed. Collision and visibility diagnostics. Tier-1 contract suite green against Isaac. **Done, 2026-09-18** — `pytest -m isaac`: 30 passed, 2 skipped, 0 failed, against the spike scene (§7.2's scope note: `full`/`style` write paths weren't covered by Spikes 1–4, and §7.4's closing note already flagged this backend for re-verification once scene v1's own materials/lights/cameras replace the spike scene — §7.6/§7.7 are that re-test, Phase 3 rework is where the backend itself gets updated). | 2 days |
| **3 rework** | Backend against the real scene | Re-verify `IsaacSceneBackend`/`writer.py`/`render.py` against scene v2 (room-inclusive, §9 Phase 2c) instead of the spike scene: `build_rig()` loads `stage_v2_room.usda` and spawns the Franka into it; `CameraL`/`CameraR` wired up as two plain `Camera` sensors (not `TiledCamera` — docs/PLAN.md); full tier-1 contract suite re-run; the zero-drift sweep (Milestone-1 carried-forward item 2) lands here permanently; `cube.y`'s signal strength re-checked against the new rig's angle; first real occlusion measurement, now that the Franka actually exists to occlude with. **Not started.** | not yet estimated |
...
**Critical path to a first defensible result: Phases 0–2c, 3 rework, 4–7.** Phases 0 through 2c are closed as of 2026-09-22; Phase 3 rework is next and not yet estimated (its scope — re-verifying the backend, wiring the new camera rig, the first real occlusion measurement — only became concrete once Phase 2c's scene and rig landed). Scene v2's room shell (Phase 2c) moved onto this path, 2026-09-21 — the first dataset is meant to be the real target scene (table + Franka + cube, in a room), not a room-less intermediate. Phases 8 and 10 remain where the scientific contribution lives, run after a first number exists to compare against.
...
`docs/PLAN.md` carries the ordered task list for the current milestone — **Phase 3 rework onward** as of 2026-09-22, Phases 0–2c having closed — including who runs what; this table stays the plan of record.
...
| Occlusion makes g non-injective; results look like encoder failure | **High, currently unmitigated** | **Revised, 2026-09-21 (docs/PLAN.md Phase 2c).** The multi-view mitigation this row used to point to is retired along with the old 3-camera rig (§5.3) — the current head-mounted stereo pair is a single viewpoint, chosen for realism, not occlusion avoidance. Per-sample visibility logging and the injectivity proxy check are still unbuilt. Untestable regardless until Phase 3 rework spawns the Franka into this still-robot-less static scene — an accepted, explicitly deferred risk, not a forgotten one. |
| A latent's render signal is real but weak from a given single camera, and can drop to exactly zero under enough compounding writes | Medium, open | **New, found on the pod, 2026-09-18. Revised, 2026-09-21**: the mitigation this row pointed to (a second, differently-angled camera) is retired along with the old 3-camera rig. `cube.y`'s signal measured at roughly a third of `cube.x`'s magnitude for the same-size move (that camera's oblique angle put Y-motion closer to its own viewing axis) and was observed hitting bit-exact `0.0` in one long test session under enough simultaneous attribute writes — a genuine per-view weak-signal robustness gap, not the render-freezing bug §5.5 already fixed. Whether the new head-stereo rig's different (closer-to-top-down) angle helps, hurts, or does nothing to this signal is untested — deliberately deferred per the camera redesign's own scope (§5.3), not assumed fixed. Test coverage: `tests/contract/test_backend_contract.py::test_base_knobs_are_all_sensitive`, added specifically because the pre-existing tests could (and did) pass on the `full`/`style` dims' signal alone while `base` dims stayed silently disconnected underneath. |
...
| `full`/`style` attribute write paths are not bitwise-deterministic under `standard` | **Resolved against scene v1 (§7.6) and scene v2 (§7.7) for `cube.size`/`cube.hue`/`light.intensity`/`light.warmth`/`table.roughness`/`table.albedo`/`light.azimuth_elevation` — all seven now bitwise-clean and usable. Medium, open for `cam.jitter`'s residual, which scales with scene complexity** | **Fixed for six of seven, resolved for the seventh.** `table.roughness`, previously a confirmed dead knob against the old scene's flat material/lighting, is now clearly responsive — the old finding was scene-specific, not structural. `light.azimuth_elevation` went from ~2,500–120,000× weaker than every other knob (scene v1, §7.6) to squarely in the same range as the rest (`mad_vs_base` 1.69, scene v2, §7.7) — the room gives the `KeyLight`'s rotation real surfaces to change the look of, resolving this without touching the light itself. `cam.jitter` is strongly responsive but has a residual non-determinism that scales with scene complexity — small against scene v1 (`mad` 4.07×10⁻⁵), confirmed via controlled A/B to be specifically room-caused and much larger against scene v2 (`mad` 0.4427) — distinct from every attribute-write knob here, all of which are bitwise-clean against the room. Not yet root-caused; see the dedicated risk row below. |
| `cam.jitter`'s camera-pose write path has a residual non-determinism, and it scales with scene complexity | **Escalated, §7.7, 2026-09-22. Medium — confirmed room-caused via a controlled A/B, not yet root-caused.** §7.6 first measured this against scene v1 as `mad` 4.07×10⁻⁵ (~2 pixel-units out of 49,152) and rated it Low. Against scene v2 (room), the same check's `back_to_back_mad` measured **0.4427** — four orders of magnitude larger. A `--stage v1|v2` toggle added to `spike_dynamic_attrs.py` isolated the cause: re-running with the *identical* camera position against the room-less stage dropped it straight back to `2.035×10⁻⁵` (the same order as §7.6's original finding), so this is specifically the room's added geometry, not the Phase 2c camera-rig redesign that happened to land in the same phase. Order-independence stays bitwise-clean (`0.0`) either way — only *repeating* an identical jittered pose shows it. Every attribute-write knob (light/material/geometry) stays fully bitwise-clean against the room; only the camera-pose write path is sensitive to it. Mechanism still not confirmed (plausibly path-traced indirect-light/edge-sampling variance at the jittered view, under-compensated by whatever resets accumulation between writes) — worth root-causing before `cam.jitter` is trusted at dataset-generation scale, not blocking Phase 2c's close-out since it is a `style`-only latent. |
...
| `cube.size` confounds with camera distance under a single view | Medium | **Revised, 2026-09-21.** A larger cube further away renders near-identically to a smaller one nearer — non-injectivity of the same kind as §5.3's occlusion, introduced by the `full` group. This is the concrete motivation for the current head-stereo rig (§3.1, §5.3): real depth-from-disparity from a fixed baseline, not multiple independent viewpoints, is the intended fix, since two lenses at one head position resolve scale/distance directly rather than relying on a second camera happening to see it differently. Not yet empirically confirmed against this rig; the injectivity proxy in §10.1 is what would catch it either way. |
