...
| **Squash radii for every `full` and `style` handle** | Measured ranges from the same spike, plus scene v1/v2 for anything table-relative. `cube.size`'s upper bound is already pinned by the *measured* gripper aperture (§5.2.2); the determinism blocker (§7.5/§7.6/§7.7) is resolved for every handle now, including `table.roughness` and `light.azimuth_elevation` — only `cam.jitter`'s residual remains open. | Phase 3 / Phase 2 |
