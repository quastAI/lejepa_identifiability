...
| **2c** | Scene v2 (room) — **reordered ahead of Phase 6, 2026-09-21** | USD reference of an empty room shell around stage v1. **Room shell authored and locally tested, 2026-09-21** — `idtb/scenegen/stage_v2_room.py` composes `scenes/stage_v1_tabletop.usda`'s `/World` onto its own `/World` by reference (final size 3×3 m floor, 2.5 m ceiling, centered on `CUBE_XY`, clears every camera eye with ≥0.5 m margin — §3.1), regenerated into `scenes/stage_v2_room.usda`, structurally verified via `usd-core` (`tests/test_scenegen.py`). Dome light stays textureless, deferred (§3.1/§3.2) — not re-authored. **Camera rig fully redesigned during this phase's pod session**, not just the room: the original `Camera1`/`Camera2`/`Camera3` rig is retired in favor of a single realistic head-mounted stereo pair (`CameraL`/`CameraR`, §3.1, §5.3) — three real bugs found and fixed along the way (schema-default `clippingRange` clipping the near foreground/sitting on its 1 m boundary; the retired rig's cameras were too close to the table for that default; and the schema-default lens was a ~24° telephoto FOV cropping the render to a ~0.5×0.5 m patch, fixed to a realistic ~69° RealSense-D435-class FOV). Pod-confirmed clean, 2026-09-21 (`docs/pod_sessions/2026-09-21_scene_v2_view/`). **Still open**: re-checking Phase 2b's `table.albedo`/`table.roughness`/`light.azimuth_elevation` verdicts, and `cube.y`'s signal strength, against this new rig's lighting/angle; occlusion is an accepted deferred risk, untestable until Phase 3 rework spawns the Franka. | 4–6 days |
