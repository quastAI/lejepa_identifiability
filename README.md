...
| **2c** | Scene v2 (room) — **reordered ahead of Phase 6, 2026-09-21** | USD reference of an empty room shell around stage v1. **Authored and locally tested, 2026-09-21** — `idtb/scenegen/stage_v2_room.py` composes `scenes/stage_v1_tabletop.usda`'s `/World` onto its own `/World` by reference (5×5 m floor, 3 m ceiling, centered on `CUBE_XY`, clears all 3 camera eyes with ≥0.5 m margin — §3.1), regenerated into `scenes/stage_v2_room.usda`, structurally verified via `usd-core` (`tests/test_scenegen.py`). Dome light stays textureless, deferred (§3.1/§3.2) — not re-authored. **Still open**: the pod-session visual check (`spikes/spike_scene_v2_view.py`, written but not yet run) and re-checking Phase 2b's `table.albedo`/`table.roughness`/`light.azimuth_elevation` verdicts against the room's lighting. | 4–6 days |
