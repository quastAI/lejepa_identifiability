...
- [ ] 🤖 **A minimal Isaac-facing `SceneBackend`** — *newly identified while closing out the items above, not in the original checklist.* The suite above is written to run unchanged against a second backend registered in `tests/contract/conftest.py`, but that backend doesn't exist yet, and building the "obvious" one — wiring every §5.2 role into USD/carb calls the way `spikes/spike_dynamic_attrs.py` did — would mean either faking a write path no spike has confirmed (`table.roughness`/`table.albedo`, never spiked at all) or one that's confirmed **blocked** (`light.azimuth`/`light.elevation`, README §7.5). Both are exactly the "resolve, don't guess" discipline this project has held to since Phase 3. The honest-scoped version: declare support for only what §7.2/§7.5 actually verified — `base` (joint + root-state writes) plus `cube.size`, `cube.hue`, `light.intensity`, `light.warmth`, `cam.jitter.x/y`, `exposure` (all confirmed bitwise-deterministic and responsive under `standard` + `resetPtAccumOnAnimTimeChange`) — and let `bind()` reject `table.*`/`light.azimuth`/`light.elevation` via `UnsupportedRoleError` exactly as designed. Also worth noting: §7.4's "Known limit" already predicts this exact backend gets rewritten once scene v1 replaces the spike's single `DistantLight` with an HDRI dome + area lights, which is a further reason this was not built speculatively ahead of that need.
- [ ] 🧑 **Final short pod session:** once the item above lands, run the suite against Isaac, paste results
