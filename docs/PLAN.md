...
- [x] 🧑🤖 **Third run confirmed the depth-64 re-run showed zero improvement — traced to two real bugs in the spike's own measurement, not the renderer. Both fixed:**
  ...
  - **Remaining before Spike 1/3 can be recorded in §3:** re-run with these fixes. Expect `sensitivity_arm_and_cube` to pass now that it's testing under a preset with real signal; expect `determinism_and_aliasing` to still FAIL, but *only* on `buffers_aliased` — that FAIL should be treated as permanent and correct, not something to chase away, per the same reasoning as the `/isaac-sim/kit` NOTE in `bootstrap.sh`. Then run the script a **second** time (same settings) so `cross_process_determinism` actually completes instead of resetting its baseline again.
