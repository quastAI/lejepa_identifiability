...
- [ ] 🧑 **Re-run `isaaclab.sh -p -m pytest tests -m isaac -s > /tmp/isaac_pytest.log 2>&1` from `/idtb/repo`** (after pulling this fix), then `sed -n '3200,$p' /tmp/isaac_pytest.log` for the summary tail directly (line numbers shift a little each run; grepping `"short test summary"` first to find the exact start is more reliable than a fixed range).
