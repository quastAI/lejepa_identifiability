...
- [x] 🧑🤖 **Third pod run — the session-scope fix landed, but the identical symptom recurred, which falsified the second run's diagnosis and pointed at the real cause.** Same instant burst of `E`s, same silent death, same missing summary — but this time the *timing* was the tell: 28 tests erroring within about a second cannot be 28 sequential ~10s scene builds (which the second run's "repeated `InteractiveScene` construction" theory required), yet the scene demonstrably only built once (one joint table printed, in both runs). That is only consistent with all 28 tests actually running fine and fast against the one shared scene, and something *after* the test loop — session-fixture teardown — killing the process before pytest could print anything.
  - **Real root cause: `_simulation_app`'s teardown called `app.close()`, and `SimulationApp.close()` can hard-terminate the process without returning control to Python.** This matches the very first standalone-script diagnostic exactly: it printed `BOOTED OK` but never `CLOSED OK`, which was noted at the time as "expected and harmless" -- true for a script whose next action was exiting anyway, but fatal for a pytest session that still needs to run its terminal reporter *after* that fixture tears down.
  - **Fixed:** `_simulation_app` no longer calls `app.close()` at all. The process exits right after the pytest session ends regardless, so there was never anything here that needed a clean close -- the teardown call was actively harmful, not merely unnecessary.
- [x] 🧑🤖 **Fourth pod run — a real pytest report at last (`ERRORS`/`short test summary info` both present), and the first genuine finding about `IsaacSceneBackend`'s own scene-building code.** All 28 `isaac` tests errored identically, all at `IsaacSceneBackend.__init__` → `build_rig()`, same line every time:
  ```
  table_prim = stage.GetPrimAtPath(scene["table"].cfg.prim_path.replace(...))
  AttributeError: 'FabricFrameView' object has no attribute 'cfg'
  ```
  **Real finding:** `InteractiveScene.__getitem__` returns a bare `FabricFrameView` (no `.cfg`) for a purely static `AssetBaseCfg` entry like the table -- different from the `RigidObjectCfg`-backed cube, which resolved fine the same way. Confirms the cube's own path worked, the table's asset-type distinction was the gap.
  - **Fixed:** `build_rig()` now resolves both prim paths directly from the module's own `_CUBE_PATH`/`_TABLE_PATH` templates instead of going through `scene[...].cfg` at all -- simpler than working around the type distinction, and removes the dependency on it entirely.
- [x] 🧑🤖 **Fifth pod run — one test held out after everything else went green, and it was the biggest finding of Phase 4: read-back correctness and render correctness had silently diverged.** All 27 other `isaac` tests passed on the first re-run after the fourth run's fix; `test_distinct_states_do_not_render_identically[isaac-base]` alone failed — two dramatically different `base` states (arm rotated 65°+, cube shifted ~0.23 m) rendered **bit-for-bit identical** frames, even though `read_state()` correctly showed the new values.
  - **Ruled out, in order, each via a targeted standalone diagnostic (`scripts/diag_render.py`, deleted once resolved) rather than the full suite, per instruction:** `sim.forward()` after the write (README's own documented flush — no effect); manually bumping the timeline's current time to force `/rtx/resetPtAccumOnAnimTimeChange` (no effect); confirming `sim.is_playing()` was `True` (it was); a discarded "settle" render before the real capture (no effect); `/physics/updateToUsd`, set both after and before scene construction (no effect either time).
  - **Isolated the shape of the bug**, not just its presence: writing *only* the arm or *only* the cube produced **exactly** `0.0` frame diff against a matched write of the same values that DID move the object, while a `cube.hue` attribute write in the same rig produced a large, real signal (max 120/255) — ruling out "the whole render pipeline is frozen" (which would have cast doubt on every previously-passing attribute-sensitivity test) and confirming the freeze was specific to writes made through the PhysX tensor API (`write_joint_state_to_sim`/`write_root_pose_to_sim`), never to a plain USD `Set()`.
  - **A research agent (web search + reading Isaac Lab's own `isaaclab_physx/physics/physx_manager.py` source) found the mechanism and prior art**, not folklore: `PhysicsManager.forward()` (what `sim.forward()` calls) only runs `update_articulations_kinematic()` + a zero-dt `_update_fabric(0.0, 0.0)`; the only thing that republishes a tensor-API write to whatever the camera's Fabric-backed render delegate reads is PhysX's own `simulate(dt, 0.0)` + `fetch_results()` cycle — which only `PhysicsManager.step()` (i.e. a real `sim.step()`) calls. Matches [isaac-sim/IsaacLab#6394](https://github.com/isaac-sim/IsaacLab/issues/6394) exactly ("poses written through the PhysX tensor API only reach the renderer during a physics step"). A lighter, zero-dynamics alternative ([#7138](https://github.com/isaac-sim/IsaacLab/pull/7138)) is open, under maintainer review, unmerged — a real step is currently the only public path.
  - **Fixed for the arm:** `write_latent_state()` now calls `rig.robot.set_joint_position_target(joint_pos)` (zero PD error at the write) followed by a real `rig.sim.step(render=False)` in place of `sim.forward()`. Confirmed on the pod: `arm_only`/`cube_only` cases went from exactly `0.0` frame diff to real, large signal (mean abs diff `8.7`, matching a 65°+ rotation), with **zero measured drift** in `read_state()` before vs. after the step.
  - **A second, independent defect surfaced once the arm was fixed:** combining the new `sim.step()` with a `cube.hue` write in the *same* spec — even with the cube's position value unchanged between writes — collapsed the hue signal from a clean max-120/255 down to noise-level (max `1`/255), regardless of write order relative to the step, extra discarded renders, or re-applying the hue write after the step. Isolating role-by-role (two independent mini-specs on one rebindable backend) found the cube's tensor-API root-pose write, not the arm's, was the actual cause — `arm+hue` stayed clean throughout. No official IsaacLab tutorial or example ever combines a kinematic pose write with a live material edit in one loop (materials are configured once, before `sim.reset()`); this is an undocumented interaction, not one there was a known workaround for.
  - **Fixed for the cube:** `cube.x`/`cube.y` no longer call `write_root_pose_to_sim`/`write_root_velocity_to_sim` at all. `writer.py` gained `_write_cube_position()`/`read_cube_position()`, a translate xform op on `cube_prim` — the same mechanism `cube.size`'s scale op already used reliably — with `_find_or_add_xform_op()` extended to keep a fixed `[translate, scale]` op order regardless of which role is written first (scale applied around the cube's own origin, then translated). `scene.py` gained `Rig.default_cube_xy` (from `_CUBE_XY`) as the "held at handle centre" fallback, matching `cube.size`'s existing pattern; `diagnostics()`'s collision proxy now reads cube position via `read_cube_position()` instead of the permanently-stale `cube.data.root_pos_w` tensor buffer; the now-unused `Rig.cube` field was dropped.
  - **Final result:** `isaaclab.sh -p -m pytest tests -m isaac` — **26 passed, 2 skipped** (style dims inactive for the `base` group config, expected), **0 failed.** Local `pytest`/`ruff check` also both clean.
...
- [ ] 🤖 **Sweep `write_latent_state()`'s zero-drift claim broadly before dataset generation trusts it (README §5.5).** Phase 4's fifth pod run measured zero `read_state()` drift across the states the contract suite happens to write — well-separated corner values, never an interpenetrating or near-joint-limit configuration. The step is a fixed, zero-integration-in-effect republish *in the tested range*, not proven zero-drift for every reachable state; §5.5's concern about a step introducing a many-to-one settling map is reopened, not closed, for the untested part of the latent space. Before generating a real dataset: sample broadly (including deliberately interpenetrating arm-cube states) and assert `read_state()` before/after the step matches to the same `1×10⁻⁴` tolerance the read-back gate already uses.
