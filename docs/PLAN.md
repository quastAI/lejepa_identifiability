...
- [ ] 🧑 **Re-run `isaaclab.sh -p -m pytest tests -m isaac -s` from `/idtb/repo`** (after pulling this fix) and paste the result — first real attempt at actually exercising `IsaacSceneBackend`'s scene-building code, not just getting it to boot.
