...
- [ ] 🧑 **Final short pod session:** run `pytest -m isaac` (tier-2 + the Isaac half of tier 1) against the module above, paste whatever breaks first — expect at least one iteration, same as every other Isaac-facing file in this project's history.
