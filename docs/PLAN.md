...
- [ ] 🧑 **Re-run `isaaclab.sh -p -m pytest tests -m isaac -s > /tmp/isaac_pytest.log 2>&1` from `/idtb/repo`** (after pulling this fix), then `grep -n "====\|passed\|failed" /tmp/isaac_pytest.log` — this should finally be the run where a real pytest summary line exists to find.
